...
from .siren import SirenNetwork, LayerActivation, SirenWrapperG, SirenWrapperD
...
def simulate_attack(output):
    """Simulates an 'attack' on an image tensor."""
    possible_attacks = ["multiply", "color jitter", "sharpness", "horizontal"]
    attack_chosen = random.choice(possible_attacks)

    if attack_chosen == "multiply":
        mult_factor = random.uniform(0.75, 1.5)
        output = output * mult_factor

    if attack_chosen == "color jitter":
        jitter = T.ColorJitter(brightness=0.01, contrast=0.01, saturation=0.01, hue=0.01)
        output = jitter(output)

    if attack_chosen == "sharpness":
        sharp = T.RandomAdjustSharpness(random.uniform(0.66, 1.33))
        output = sharp(output)

    if attack_chosen == "horizontal":
        hflip = T.RandomHorizontalFlip(p=1.)
        output = hflip(output)

    #print(f"{siren_name} is hit by a {attack_chosen} attack!")
    return output
...
class DeepDaze(nn.Module):
    def __init__(
        ...
        #Establish both SIRENS

        self.sirenG = SirenWrapperG(
            sG,
            [self.image_width, self.image_width, 3]
        )

        self.sirenD = SirenWrapperD(
            sD,
            [512]
        )
        ...
        self.optimG = optim.Adam(self.sirenG.parameters(), lr=lr, betas=(0.5, 0.999))
        self.optimD = optim.Adam(self.sirenD.parameters(), lr=lr, betas=(0.5, 0.999))
    ...
    def calc_image_embed(self, output):
        """Calculates loss through cosine similarity for a singular target"""
        #print(f"Outputs normalized")
        # sample cutout sizes between lower and upper bound
        output = output[None, :, :, :].permute(0, 3, 1, 2)
    ...
    def calc_loss(self, text_embed, image_embed):
        avg_image_embed = image_embed.mean(dim=0).unsqueeze(0)
        averaged_loss = -1 * torch.cosine_similarity(text_embed, avg_image_embed, dim=-1).mean()
        ...
        general_loss = -1 * torch.cosine_similarity(text_embed, image_embed, dim=-1).mean()
    ...
    def forward(self, text_embed, return_loss=True, dry_run=False):
        ...
        real_target = torch.ones((1, self.image_width)).to(device)
        fake_target = torch.zeros((1, self.image_width)).to(device)

        output = self.sirenD(coords=text_embed)
        output = output[None, :]
        #print(f"text_embed: {text_embed.shape}\noutput shape = {output.shape}")

        D_real_loss = self.loss_fn(output, real_target)
        D_real_loss.backward()

        #shape of
        image_noise = torch.randn(self.image_width, self.image_width, 3).to(device)
        #print(f"image_noise shape: {image_noise.shape}")
        generated_image = self.sirenG(coords=image_noise)

        generated_embeddings = self.calc_image_embed(generated_image)
        #print(f"generated embeddings shape: {generated_embeddings.shape}")

        output = self.sirenD(coords=generated_embeddings.detach())
        output = output[None, :]
        D_fake_loss = self.loss_fn(output, fake_target)
        ...
        gen_output = self.sirenD(coords=generated_embeddings)
        gen_output = gen_output[None, :]
        G_loss = self.loss_fn(gen_output, real_target)

        G_loss.backward()
        self.optimG.step()

        generated_image = generated_image[None, :, :, :].permute(0, 3, 1, 2)

        return generated_image, D_fake_loss, G_loss


######################################
#BORDER TO SEPARATE THIS STUFF EASIER#
######################################
...
class Imagine(nn.Module):
    ...
    def train_step(self, epoch, iteration):
        ...
        with autocast(enabled=True):
            out, loss1, loss2 = self.model(self.clip_encoding)
        ...
        ##TODO: grab just output winning image and statistics

        if iteration % self.save_every == 0:
          self.save_image(epoch, iteration, img=out, progress=self.save_progress)
          if self.save_best and loss1 < self.best_loss:
